...
var ATTR_BOUNDINGBOX = 'boundingBox',
	...
	MyItem = Y.Base.create('labr-view-item', Y.Widget, [Y.WidgetChild], {
		...
		CONTENT_TEMPLATE: '<div class="item-icon pull-left"></div>' + '<h5 class="list-group-item-heading">' + '<span>Tros Burgers</span>' + '<div class= "item-rating pull-right" >' + '<span class="glyphicon glyphicon-star"></span>' + '</div>' + '</h5>' + '<div>' + '<span class="list-group-item-text categories">Fast Food, Restaurant Food</span>' + '<span class="list-group-item-text distance pull-right">0.4 millas</span>' + '<div class="clearfix"></div>' + '</div>',
		...
		applyFilter: function (query) {
			...
			if (query === '') {
				...
				Y.each(queryArray, function (item) {
					filtered = Y.AutoCompleteFilters.subWordMatch(item, [{
						text: categories
					}]).length;
					if (filtered) {
