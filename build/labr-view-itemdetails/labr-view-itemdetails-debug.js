YUI.add('labr-view-itemdetails', function (Y, NAME) {
...
var ATTR_CONTENTBOX = 'contentBox',
    ...
    CONTENT_TEMPLATE : 	'<div class="arrow"></div>' +
						'<h3 class="popover-title item-details-title">Juicy Burger</h3>' +
						'<div class="popover-content item-details-content">' +
							'<div class="row">'+
								'<div class="col-md-7 col-xs-7 item-details-hero">'+
								'</div>'+
								'<div class="col-md-5 col-xs-5 item-details-subdetails">'+
								'</div>'+
							'</div>'+
							'<div class="row item-details-subdetails-hidden hidden">'+
							'</div>'+
						'</div>',
	...
	renderUI : function () {
		var instance = this,
			...
			subdetailNode,
			hiddenSubDetails = instance.get('hiddenSubDetails'),
	...
	syncData: function(data){
		...
		Y.each(data, function(item){
			boundingBox.one(item.selector).set('innerHTML', item.value);
		});
	}
...
}, '@VERSION@', {"requires": ["widget", "event-outside"], "skinnable": true});
